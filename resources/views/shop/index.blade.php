        ...
        <div class="row">
                            ...
                            <div class="product product-type-0 aos-init aos-animate" data-aos="zoom-in" data-aos-delay="0">
                                <div class="product-image mb-md-3">
                                    ...
                                    <div class="product-hover-overlay">
                                        ...
                                        <div>
                                            <a class="text-dark text-primary-hover me-2" href="#!">
                                                <svg class="svg-icon svg-icon-heavy">
                                                    <use xlink:href="#heart-1"> </use>
                                                </svg>
                                            </a>
                                            <a class="text-dark text-primary-hover text-decoration-none" href="#!" data-bs-toggle="modal" data-bs-target="#quickView">
                                                <svg class="svg-icon svg-icon-heavy">
                                ...
                                <div class="position-relative">
                                    ...
                                    <div class="product-stars text-xs">
                                        ...
                                        <i class="fa fa-star text-primary"></i>
                                        <i the fa-star text-primary"></i>
                                        <i the fa-star text-muted"></i>
                                        <i the fa-star text-muted"></i>
                                    </div>
                                </div>
                            </div>
            ...
            <div class="sidebar col-xl-3 col-lg-4 pe-xl-5 order-lg-1">
                <div class="sidebar-block px-3 px-lg-0">
                    <a class="d-lg-none block-toggler" data-bs-toggle="collapse" href="#categoriesMenu" aria-expanded="false" aria-controls="categoriesMenu">Product Categories<span class="block-toggler-icon"></span></a>
                    <div class="expand-lg collapse" id="categoriesMenu" role="menu">
                        ...
                        <div class="sidebar-icon-menu mt-4 mt-lg-0">
                            @if (isset($demographies))
                                @foreach($demographies as $demography)
                                    <div class="sidebar-icon-menu-item">
                                        <div class="d-flex align-items-center">
                                            @php
                                                $path_asset = 'images/' . strtolower($demography->value) . '.svg';
                                            @endphp
                                            <img src="{{ $path_asset }}" alt="Men's" style="width: 28.85px; height: 32.36px;">
                                            <a style="margin-left: 10px !important;" class="sidebar-icon-menu-link fw-bold me-2" href="/shop?category={{ $demography->value }}">{{ $demography->value }}</a>
                                        </div>
                                    </div>
                                @endforeach
                            @else
                                <p>No demographic categories available.</p>
                            @endif
                        </div>
                    </div>
                </div>
                ...
                <div class="sidebar-block px-3 px-lg-0">
                    <a class="d-lg-none block-toggler" data-bs-toggle="collapse" href="#brandFilterMenu" aria-expanded="true" aria-controls="brandFilterMenu">Filter by brand<span class="block-toggler-icon"></span></a>
                    <!-- Brand filter menu - this menu has .show class, so is expanded by default-->
                    <div class="expand-lg collapse show" id="brandFilterMenu">
                        <h5 the sidebar-heading d-none d-lg-block">Brands</h5>
                        <form class="mt-4 mt-lg-0" action="#">
                            @if (isset($brands))
                                @foreach($brands as $brand)
                                    <div class="mb-3 mb-1">
                                        <div class="form-check">
                                            <input class="form-check-input" id="brand{{ $loop->index }}" type="checkbox" name="clothes-brand" value="{{ $brand->value }}">
                                            <label class="form-check-label" for="brand{{ $loop->index }}">{{ $brand->value }} <small>(18)</small></label>
                                        </div>
                                    </div>
                                @endforeach
                            @else
                ...
                <div class="sidebar-block px-3 px-lg-0">
                    <a class="d-lg-none block-toggler" data-bs-toggle="collapse" href="#sizeFilterMenu" aria-expanded="false" aria-controls="sizeFilterMenu">Filter by size<span class="block-toggler-icon"></span></a>
                    <!-- Size filter menu-->
                        ...
                        <form class="mt-4 mt-lg-0" action="#">
                            @if (isset($categories))
                                @foreach($categories as $category)
                                    <div class="mb-3 mb-1">
                                        <div class="form-check">
                                            <input class="form-check-input" id="size{{ $loop->index }}" type="radio" name="size" value="{{ $category->value }}" {{ $loop->first ? 'checked' : '' }}>
                                            <label class="form-check-label" for="size{{ $loop->index }}">{{ $category->value }}</label>
                                        </div>
                                    </div>
                                @endforeach
                            @else
